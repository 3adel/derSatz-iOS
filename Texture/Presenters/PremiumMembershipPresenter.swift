...
class PremiumMembershipPresenter: Presenter, PremiumMembershipPresenterProtocol {
    ...
    override func getInitialData() {
        ...
        iapService.retrievePrice(for: DerSatzIAProduct.premium) { [weak self] priceString in
            guard let priceString = priceString else { return }
            self?.populateView(priceString: priceString)
        }
    }
    ...
    private func populateView(priceString: String = "$7.99") {
        ...
        let demoViewModel = PremiumMembershipViewModel(title: title,
                                                       body: body,
                                                       buyButtonTitle: "Unlock Now for \(priceString)")
